...
parser.add_argument('--dir', default='Dog-Data',
                     help='dataset directory')
# add_argument 메서드를 사용하여 명령행 인수를 정의합니다. = argument 추가 
# 이 예제에서는 --dir 인수를 정의하고, 기본값으로 'Dog-Data'를 설정하고, 인수에 대한 설명을 제공합니다.
# 즉, 사용자가 --dir로 값을 지정하지 않으면 기본값인 'Dog-Data'가 사용됩니다.
...
def read_data(label2id):
    ...
    for label in os.listdir('Dog-Data/train'):
        ...
        if os.path.isdir(os.path.join('Dog-Data/train', label)):
            ...
            for img_file in os.listdir(os.path.join('Dog-Data/train',
                                                     label)):
                # 이미지를 읽어옵니다.
                img = cv2.imread(os.path.join('Dog-Data/train', 
                                              label, img_file)) 
...
def main():
    ...
    # BOW 딕셔너리 존재 유무 체크 후 생성 또는 로드 # 경로 변경 필요
    if not os.path.isfile('Dog-Data/bow.pkl'):  # 저장된 BoW 딕셔너리 파일이 존재하지 않는 경우
        BoW = kmeans_bow(all_descriptors, num_clusters)  # K-means를 사용하여 BoW 딕셔너리 생성
        pickle.dump(BoW, open('Dog-Data/bow.pkl', 'wb'))  # 생성된 BoW 딕셔너리를 파일에 저장
    else:
        BoW = pickle.load(open('Dog-Data/bow.pkl', 'rb'))  # 저장된 BoW 딕셔너리 파일을 로드합니다.
...
# 경로 변경 필요
    test_dir = '/Users/kownkihoon/Desktop/conpitition/dognose/Save-Pets-ML-main/SVM-Classifier/Dog-Data/test/'
