...
# def find_images(image_paths, img_extensions=['.jpg', '.png', '.jpeg']):
#     img_extensions += [i.upper() for i in img_extensions]
#     image_list = []  # Store the image paths in a list
...
#                 logging.info(f'{path.suffix} is not an image extension! skipping {path}')
#             else:
#                 image_list.append(path)  # Append the image path to the list

#         if path.is_dir():
#             for img_ext in img_extensions:
#                 image_list.extend(path.rglob(f'*{img_ext}'))  # Extend the list with image paths
